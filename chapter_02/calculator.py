...
index = input('Enter the number corresponding to the operator to run (press enter to run all operators): ')
...
if index.strip().isdigit():                                                  # Is the input a number? If it is, get the corresponding operator function and terms from the data and calculate the result
    ...
    if index > 0 and index <= len(operators)-1:                              # is the input number within the range of operator_functions provided?
        ...
        print(f'Please choose a number between 1 and {len(operator)-1}')
